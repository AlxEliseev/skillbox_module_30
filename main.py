...
@app.post(
    "/recipes/",
    response_model=schemas.RecipeOutDetailed,
...
async def recipes(recipe: schemas.RecipeIn) -> models.Recipe:
    ...
    for ingredient_id in recipe_.pop("ingredients"):
        ...
        if ingredient:
            ...
            raise HTTPException(
                400, f"No ingredient with id # {ingredient_id}. Create ingredient first"
            )
...
@app.get(
    "/recipes/",
    response_model=List[schemas.RecipeOutShort],
...
async def get_recipes() -> List[models.Recipe]:
    res = await session.execute(
        select(models.Recipe).order_by(
            models.Recipe.views.desc(), models.Recipe.cooking_time.desc()
        )
    )
